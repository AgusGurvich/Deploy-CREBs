...
router.get("/pagos", auth.isLoggedIn, auth.isBecario, async (req,res)=> {
...
router.get('/pagoIndividual/:pago_id', auth.isBecario, async (req, res)=> {
    // Buscar el usuario
    const pago_id = req.params.pago_id;
    const pagoQuery = 'SELECT * FROM pagos where id = ?';
    const resultadoPago = await pool.query(pagoQuery, [pago_id]);
    const pago = resultadoPago[0][0];
    //const personaQuery = 'SELECT * FROM users WHERE ID = ?'
    //const resultadoPersona = await pool.query(personaQuery, [pago.user_id])
    //const persona = resultadoPersona[0][0];
  
    res.render('acreditarPago', {
        pago : pago,
        //persona : persona 
    })
}) 
...
router.put("/nuevaCargaPago", auth.isBecario , async (req,res)=> {
    // Definiciones
    const nuevaCarga = req.body.valor;
    const userID = req.body.id;
    ...
    const updateQuery = 'UPDATE users SET saldo = ' + nuevoSaldo + ' WHERE id = ' + userID;  
    await pool.query(updateQuery);
    // Eliminar pedido de acreditación
    const deletePagoQuery = 'DELETE FROM pagos WHERE id = ?';
    await pool.query(deletePagoQuery, [pagoID]);
    // Obtener DNI
    const redirectURL = `/pagos`;
    res.redirect(redirectURL);
})
